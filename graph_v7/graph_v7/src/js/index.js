...
.then(function(dataArray) {
    // window.addEventListener('DOMContentLoaded', function(){
        ...
        var cy2 = cytoscape({
            ...
            layout: {name: 'dagre', nodeSep: 20, edgeSep:20, rankSep: 30, rankDir:'TB', minLen: 1, edgeWeight: 3, avoidOverlap:true, handleDisconnected:true},
            style: dataArray[0],
            elements: elements
            });
        ...
        /* 更新布局 */
        function drawLayout() {
            let parentNodes = cy2.filter(function(element, i){
                return element.isNode() && (element.data('isParent') === true);
            });
            // for(let node of cy2.nodes()) {
                // 有父亲节点的，才能执行
                // if(node.data('parent')) { 
                    //父亲节点一样的节点，他们的横轴一样
                    // let pos = node.position()
                    for(let parent of parentNodes) {
                        let allSameParentNodes = cy2.filter(function(element, i){
                            return element.isNode() &&(element.data('parent') === parent.data('id'));
                        });
                        let tempPos = []
                        for(let sameNode of allSameParentNodes) {
                            sameNode.position('x', allSameParentNodes[0].position('x'))
                            tempPos.push(sameNode)
                        }
                        ...
                        //     // allSameParentNodes[i].position('x',tempPos[i].position('x'))
                        //     allSameParentNodes[i].position('y',tempPos[i].position('y'))
                        // }
                    }
                // }
